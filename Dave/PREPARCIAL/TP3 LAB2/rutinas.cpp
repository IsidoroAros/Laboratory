...
int cantRutinasUsuario(int id, int anio){
    FILE *pFile;
    Rutina regAux;
    int cantRutinas=0;

    pFile=fopen(FILE_RUTINAS,"rb");
    if(pFile==NULL){
            return - 1 ;///error de archivo
        }
    while(fread(&regAux,sizeof(Rutina),1,pFile)==1){
        if(regAux.idUsuario==id && regAux.fechaRutina.anio==anio){
            cantRutinas++;
        }
    }
    fclose(pFile);
